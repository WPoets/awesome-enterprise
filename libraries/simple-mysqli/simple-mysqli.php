...
/* 		$sql="
 SET @tkg_agent = ''; SET @exact = ''; SET @partial = '%%'; with q0 as (	SELECT object_id as data_id FROM lapp_meta where coll_id='lapp' AND meta_key = 'lapp_stage' AND meta_value = 'credit-appraisal'	) ,q1 as ( select pquery .*, date(meta_value) as received_date from q0 as pquery left join lapp_meta on pquery.data_id=lapp_meta.object_id and meta_key='turnaround_end' ),q2 as ( select q1.*, meta_value as lapp_status from q1 left join lapp_meta on q1.data_id=lapp_meta.object_id and coll_id='lapp' and meta_key='lapp_status' ),q3 as ( select q2.*, meta_value as fixed_income from q2 left join lapp_meta on q2.data_id=lapp_meta.object_id and coll_id='lapp' and meta_key='fixed_income' ),q4 as ( select q3.*, meta_value as req_scheme_id from q3 left join lapp_meta on q3.data_id=lapp_meta.object_id and coll_id='lapp' and meta_key='req_scheme_id' ),q5 as ( select q4.*, meta_value as tkg_agent from q4 left join lapp_meta on q4.data_id=lapp_meta.object_id and coll_id='lapp' and meta_key='tkg_agent' ),q6 as ( select q5.*, meta_value as recommended_to from q5 left join lapp_meta on q5.data_id=lapp_meta.object_id and coll_id='lapp' and meta_key='recommended_to' ) ,q7 as ( select q6.* from lapp_meta join q6 on q6.data_id=lapp_meta.object_id where 1=1 and lapp_meta.coll_id='lapp'	and lapp_meta.meta_key in ('lapp_status') and meta_value IN ('received') ), bounded_data as ( SELECT * FROM q7 LIMIT 300 ), junk AS (SELECT * FROM bounded_data) select count(1) from bounded_data;
  ...
  "; */
    ...
    do {
				...
				if(is_object($temp)){
					...
					//\util::var_dump($this->stmtResult->fetch_all(MYSQLI_ASSOC));
				}

				
				
			
			/* 	if($this->stmtResult!==false){
